...
var setOneShotArgs = require('./monitor/restart-cmd').setOneShotArgs;
var eventHandlers = {};
  ...
  config.load(options, function (config) {
    ...
    if (config.options.stdin && config.options.restartable) {
      ...
      bindStdinData(function onRestartableStdin(data) {
        ...
        if (parsed) {
          // plain `rs` → clear pending one-shot; `rs <args>` → queue them
          setOneShotArgs(config, parsed.extraArgs);
          var reason = {
            type: 'manual',
    ...
    if (!config.required) {
      ...
      process.on(restartSignal, function () {
        ...
        emitRestart(undefined, { type: 'signal', signal: restartSignal });
      });
      utils.bus.on('error', () => {
...
nodemon.restart = function (reason) {
  ...
  if (Array.isArray(r.args) && r.args.length) {
    setOneShotArgs(config, r.args);
  } else {
    setOneShotArgs(config, null);
  }
  emitRestart(undefined, r);
  return nodemon;
};
