...
var parseRestartLine = require('./monitor/restart-cmd').parseRestartLine;
var requestRestart = require('./monitor/restart-cmd').requestRestart;
var eventHandlers = {};

/** @type {Function[]} stdin 'data' handlers we own (removed on reset) */
var stdinDataHandlers = [];

function bindStdinData(handler) {
  process.stdin.on('data', handler);
  stdinDataHandlers.push(handler);
}

function unbindStdinData() {
  stdinDataHandlers.forEach(function (handler) {
    try {
...
function nodemon(settings) {
  ...
  config.load(options, function (config) {
    ...
    if (config.options.stdin && config.options.restartable) {
      ...
      // restartable). Also supports one-shot extra args: `rs <args>` restarts
      // once with those args appended; the next restart uses the original
      // command. Non-matching input is still piped to the child (unchanged).
      process.stdin.resume();
      process.stdin.setEncoding('utf8');
      bindStdinData(function onRestartableStdin(data) {
        var line = data.toString();
        var parsed = parseRestartLine(line, config.options.restartable);

        if (parsed) {
          // plain `rs` → original command; `rs <args>` → one-shot (validated)
          var reason = {
            type: 'manual',
            trigger: config.options.restartable,
          };
          if (parsed.extraArgs && parsed.extraArgs.length) {
            reason.args = parsed.extraArgs.slice();
          }
          requestRestart(undefined, reason);
      ...
      bindStdinData(function onRawStdin(data) {
        data = data.toString();
        buffer += data;
    ...
    if (!config.required) {
      ...
      process.on(restartSignal, function () {
        // same status line as nodemon.restart() so API / signal paths match
        utils.log.status('restarting child process');
        // signal leaves pending one-shot alone (requestRestart ownership rules)
        requestRestart(undefined, { type: 'signal', signal: restartSignal });
      });
      utils.bus.on('error', () => {
    ...
    // Opt-in MCP server: inspect status / watches / history / logs and
    // trigger restart or quit. Not loaded when mcp is false (default).
    if (config.options.mcp) {
      try {
        require('./mcp/server')
          .start(config.options, nodemon)
          .catch(function (err) {
            utils.log.error('failed to start MCP server: ' + (err && err.message));
          });
      } catch (err) {
        utils.log.error('failed to load MCP server: ' + (err && err.message));
      }
    }
 ...
 * @param {{type?:string, trigger?:string, signal?:string, files?:string[], args?:string[]}} [reason]
 *        Optional restart reason (defaults to { type: 'api' }). MCP uses
 *        { type: 'api', trigger: 'mcp' } for accurate history.
 *        Optional `args` queues one-shot extra child args for the next run only
 *        (same as typing `rs <args>`); plain restart clears any pending one-shot.
 *        Shell-unsafe args are refused (no restart).
 */
nodemon.restart = function (reason) {
  var r =
    reason && typeof reason === 'object' ? reason : { type: 'api' };
  if (!r.type) {
    r = Object.assign({ type: 'api' }, r);
  }
  // Only log restart status when the request is accepted (unsafe args refuse).
  if (requestRestart(undefined, r)) {
    utils.log.status('restarting child process');
  }
  return nodemon;
};
...
bus.on('reset', function (done) {
  debug('reset');
  // MCP stop also listens on 'reset'; call explicitly so required-mode
  // resets always free the port even if MCP was started late.
  try {
    require('./mcp/server').stop();
  } catch (e) { /* mcp not loaded */ }
  // Drop stdin handlers so required-mode re-runs do not leak 'data' listeners
  unbindStdinData();
