...
function parse(argv) {
  ...
  for (var i = 0; i < args.length; i++) {
    ...
    if (lookForArgs) {
      ...
      // Optional value for --restartLoopGuard / --restart-loop-guard:
      // only consume the next token when it looks like a guard specification
      // (e.g. "10", "10/5s", "true"), never a script path.
      if (
        args[i] === '--restartLoopGuard' ||
        args[i] === '--restart-loop-guard'
      ) {
        var guardSpec = args[i + 1];
        if (guardSpec && /^(true|false|\d+)/i.test(guardSpec)) {
          nodemonOptions.restartLoopGuard = parseRestartLoopGuard(guardSpec);
          args.splice(i, 2);
        } else {
          nodemonOptions.restartLoopGuard = true;
          args.splice(i, 1);
        }
        i--;
        continue;
...
function nodemonOption(options, arg, eatNext) {
  ...
  if (arg === '--kill-timeout' || arg === '--killTimeout') {
    options.killTimeout = parseKillTimeout(eatNext());
  } else

  if (arg === '--startUpWatchDelay' || arg === '--startup-watch-delay') {
    options.startUpWatchDelay = parseDelay(eatNext());
  } else

  if (arg === '--restartReason' || arg === '--restart-reason') {
    options.restartReason = true;
  } else

  if (arg === '--restartOn' || arg === '--restart-on') {
    options.restartOn = parseRestartOn(eatNext());
  } else

  if (arg === '--mcp') {
    // optional value: http | stdio | true
    options.mcp = true;
    options.mcpTransport = 'http';
  } else

  if (arg === '--mcp-stdio' || arg === '--mcpStdio') {
    options.mcp = true;
    options.mcpTransport = 'stdio';
  } else

  if (arg === '--mcpPort' || arg === '--mcp-port') {
    options.mcpPort = parseInt(eatNext(), 10);
  } else

  if (arg === '--mcpHost' || arg === '--mcp-host') {
    options.mcpHost = eatNext();
  } else

  if (arg === '--mcpTransport' || arg === '--mcp-transport') {
    options.mcpTransport = eatNext();
    options.mcp = true;
  } else

  if (arg === '--mcpToken' || arg === '--mcp-token') {
    options.mcpToken = eatNext();
    options.mcp = true;
  } else

  if (arg === '--mcpAllowRemote' || arg === '--mcp-allow-remote') {
    options.mcpAllowRemote = true;
    options.mcp = true;
  } else
...
/**
 * Parse --kill-timeout. Bare numbers are milliseconds (flag is <ms>).
 * Optional suffixes: "500ms", "2s". Invalid / missing → 0 (disabled).
 *
 * @param {String} value
 * @return {Number} milliseconds, or 0 when disabled/invalid
 */
function parseKillTimeout(value) {
  if (value === undefined || value === null || value === '') {
    return 0;
  }
  var str = String(value).trim();
  var millis;
  if (/^\d+(\.\d+)?ms$/i.test(str)) {
    millis = parseFloat(str);
  } else if (/^\d+(\.\d+)?s$/i.test(str)) {
    millis = parseFloat(str) * 1000;
  } else {
    // bare number = milliseconds (unlike --delay, which is seconds)
    millis = parseFloat(str);
  }
...
/**
 * Parse --restartOn values: "all", "change", "add", "unlink",
 * or comma-separated combinations (e.g. "change,add").
 *
 * @param {String} value
 * @return {string|string[]}
 */
function parseRestartOn(value) {
  if (value === undefined || value === null || value === '') {
    return 'all';
  }
  ...
  var parts = str.split(/[,\s]+/).map(function (p) {
    return p.trim();
  }).filter(Boolean);

  if (parts.length === 0 || parts.indexOf('all') !== -1) {
    return 'all';
  }
...
/**
 * Parse --restartLoopGuard values.
 *
 * Forms:
 *   "10"       -> { max: 10, window: 10000 }  (default 10s window)
 *   "10/5s"    -> { max: 10, window: 5000 }
 *   "10/5000ms"-> { max: 10, window: 5000 }
 *   "true"     -> true (defaults applied at runtime)
 *
 * @param {String} value
 * @return {boolean|{max:number,window:number}}
 */
function parseRestartLoopGuard(value) {
  if (value === undefined || value === null || value === '') {
    return true;
  }
  ...
  if (parts.length > 1 && parts[1]) {
    windowMs = parseDelay(parts[1]);
    if (!windowMs) {
      windowMs = 10000;
    }
  }

  return { max: max, window: windowMs };
}
