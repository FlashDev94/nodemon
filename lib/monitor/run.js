...
var materializeCommand = require('./restart-cmd').materializeCommand;
var child = null; // the actual child process we spawn
var killedAfterChange = false;
var noop = () => {};
var restart = null;
// Single force-kill timer for killTimeout (only one active at a time)
var forceKillTimer = null;
var forceKillExitHandler = null;
var forceKillChild = null;
...
function run(options) {
  ...
  if (!runCmd) {
    ...
    // Do not materialize/consume one-shot args until we actually start.
  ...
  // Resolve command for this run: original + optional one-shot `rs <args>`.
  // Consumes oneShotArgs so the next restart always uses the original command.
  var cmd = materializeCommand(config);
  ...
  // Optional pid arg is backward-compatible for existing `start` listeners.
  // MCP uses it so childPid is available without requiring --verbose.
  bus.emit('start', child && child.pid);

  // After the child starts, optionally ignore file changes for a short window
  // so apps that write generated files on startup do not trigger a restart loop.
  // This is separate from `delay`, which debounces restarts after a file change.
  if (config.options.startUpWatchDelay > 0) {
    config.ignoreWatchUntil = Date.now() + config.options.startUpWatchDelay;
    utils.log.detail(
      'ignoring file changes for ' +
        config.options.startUpWatchDelay +
        'ms after startup'
    );
  } else {
    config.ignoreWatchUntil = 0;
  }
  ...
  child.on('exit', function (code, signal) {
    ...
    if (signal === config.signal || code === 0) {
      ...
      // Optional exit code is backward-compatible for existing `crash` listeners.
      bus.emit('crash', code);
...
/**
 * killTimeout option in ms. 0 / unset / invalid → disabled (legacy behavior).
 * @return {number}
 */
function getKillTimeoutMs() {
  var t = config.options && config.options.killTimeout;
  if (t === undefined || t === null || t === false) {
    return 0;
  }
...
/**
 * Force-kill the child and its process tree (SIGKILL / taskkill /F).
 * Used only when killTimeout is set and the graceful signal did not work.
 */
function forceKillTree(child, pid) {
  if (utils.isWindows) {
    try {
      exec('taskkill /pid ' + pid + ' /T /F');
    } catch (e) {
      utils.log.error('Could not force-kill sub process');
    }
    return;
  }

  var kill9 = function (p) {
    try {
...
/**
 * Clear any pending killTimeout force-kill timer / exit watcher.
 */
function clearForceKillTimer() {
  ...
  if (forceKillExitHandler && forceKillChild) {
    try {
      forceKillChild.removeListener('exit', forceKillExitHandler);
    } catch (e) { /* ignore */ }
  }
  forceKillExitHandler = null;
  forceKillChild = null;
}

/**
 * When killTimeout > 0, if the child is still alive after the timeout following
 * the graceful signal, force SIGKILL. If the child exits in time, the timer is
 * cleared and no force kill runs. When killTimeout is off, this is a no-op so
 * behavior matches historical nodemon.
 *
 * Only one force-kill timer is active at a time (re-arming cancels the previous).
 *
 * @param {import('child_process').ChildProcess|null} childProc
 */
function armKillTimeout(childProc) {
  // Cancel any previous arm so rapid restarts cannot stack SIGKILL timers
  clearForceKillTimer();

  var timeoutMs = getKillTimeoutMs();
  if (!timeoutMs || !childProc || !childProc.pid) {
    return;
  }

  var pid = childProc.pid;
  forceKillChild = childProc;

  forceKillExitHandler = function () {
    clearForceKillTimer();
  };

  childProc.once('exit', forceKillExitHandler);
  ...
  forceKillTimer = setTimeout(function () {
    ...
    if (!isPidAlive(pid)) {
      debug('killTimeout fired but pid %s already exited', pid);
      return;
    }

    utils.log.status(
      'process has not exited within killTimeout (' +
        timeoutMs +
        'ms), sending SIGKILL...'
    );
    forceKillTree(childProc, pid);
  }, timeoutMs);
}
...
function kill(child, signal, callback) {
  ...
  // Opt-in: force SIGKILL if process ignores the graceful signal too long.
  // When killTimeout is 0/unset, armKillTimeout is a no-op (legacy behavior).
  // Re-arming cancels any previous force-kill timer for this monitor.
  armKillTimeout(child);
