...
var snapshotCommandBase = require('../monitor/restart-cmd').snapshotCommandBase;
...
function reset() {
  ...
  config.loaded = [];
  // permanent original command + pending one-shot `rs <args>` extras
  config.commandBase = null;
...
config.load = function (settings, ready) {
  ...
  load(settings, config.options, config, function (options) {
    ...
    // commandBase is never mutated by one-shot `rs <args>`; materializeCommand
    // rebuilds config.command from this snapshot on every run.
    config.commandBase = snapshotCommandBase(cmd);
    config.oneShotArgs = null;
    config.command = {
      raw: {
