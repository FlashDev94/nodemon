...
function defineTools(nodemonApi) {
  return [
    ...
    },
    {
      ...
      handler: async function () {
        if (nodemonApi && typeof nodemonApi.restart === 'function') {
          ...
          // Prefer the restart-reason path when available so history is accurate.
          try {
            require('../monitor/restart-reason').emitRestart(undefined, {
              type: 'api',
              trigger: 'mcp',
            });
          } catch (e) {
    ...
    },
    {
      ...
      handler: async function () {
        // delay so HTTP/MCP can flush the response before teardown
        setTimeout(function () {
          utils.bus.emit('quit');
        }, 50);
        return textResult({
