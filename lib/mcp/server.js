...
function defineTools(nodemonApi) {
  return [
    ...
    },
    {
      ...
      handler: async function () {
        // Same path as API restart: clears pending one-shot, consistent history
        if (nodemonApi && typeof nodemonApi.restart === 'function') {
          nodemonApi.restart({ type: 'api', trigger: 'mcp' });
        } else {
          require('../monitor/restart-cmd').requestRestart(undefined, {
            type: 'api',
            trigger: 'mcp',
          });
        }
